    ...
    try {
        return await withConnection(async (connection) => {
            let sql = `
                ...
                FROM vultrack.poamapprovers T1
                INNER JOIN vultrack.user T2 ON T1.userId = T2.userId
                WHERE poamId = ?;
            `;
    ...
    try {
        return await withConnection(async (connection) => {
            let sql = `
                ...
                FROM vultrack.poamapprovers T1
                INNER JOIN vultrack.user T2 ON T1.userId = T2.userId
                INNER JOIN vultrack.poam T3 ON T1.poamId = T3.poamId
...
exports.postPoamApprover = async function postPoamApprover(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql_query = `
                INSERT INTO vultrack.poamapprovers (poamId, userId, approvalStatus, approvedDate, comments)
                VALUES (?, ?, ?, ?, ?)
            `;
            ...
            if (req.body.poamLog[0].userId) {
                let userSql = "SELECT fullName FROM vultrack.user WHERE userId = ?";
            ...
            let sql = "SELECT * FROM vultrack.poamapprovers WHERE poamId = ? AND userId = ?";
        ...
        if (error.code === 'ER_DUP_ENTRY') {
            return await withConnection(async (connection) => {
                let fetchSql = "SELECT * FROM vultrack.poamapprovers WHERE poamId = ? AND userId = ?";
    ...
    try {
        return await withConnection(async (connection) => {
            ...
            if (req.body.approvalStatus === 'Rejected') {
                sql_query = "UPDATE vultrack.poam SET status = ? WHERE poamId = ?;";
                await connection.query(sql_query, ["Rejected", req.body.poamId]);
                const poamSql = "SELECT submitterId FROM vultrack.poam WHERE poamId = ?";
                ...
                const poamSql = "SELECT submitterId, rawSeverity, collectionId FROM vultrack.poam WHERE poamId = ?";
                const [poamResult] = await connection.query(poamSql, [req.body.poamId]);
                const submitterId = poamResult[0].submitterId;
                ...
                const permissionSql = "SELECT userId FROM vultrack.collectionpermissions WHERE collectionId = ? AND accessLevel = 4";
                ...
                if (rawSeverity === "CAT I - Critical/High") {
                    ...
                    const cat1NotificationSql = `INSERT INTO vultrack.notification (userId, title, message) VALUES (?, ?, ?)`;
...
exports.deletePoamApprover = async function deletePoamApprover(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql = "DELETE FROM vultrack.poamapprovers WHERE poamId = ? AND userId = ?";
