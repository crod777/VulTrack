    ...
    if (userNameInput == "Registrant") {
        try {
            return await withConnection(async (connection) => {
                let sql = "SELECT collectionId, collectionName FROM vultrack.collection;"
...
exports.postCollection = async function postCollection(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql_query = `INSERT INTO vultrack.collection (collectionName, description, assetCount, poamCount) VALUES (?, ?, ?, ?) `
            await connection.query(sql_query, [req.body.collectionName, req.body.description, 0, 0, 0])
            let sql = "SELECT * FROM vultrack.collection WHERE collectionId = LAST_INSERT_ID();"
...
exports.putCollection = async function putCollection(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql_query = "UPDATE vultrack.collection SET collectionName=?, description=?, assetCount= ?, poamCount= ? WHERE collectionId = ?";
