...
exports.getAvailablePoams = async function getAvailablePoams(req, res, next) {
    try {
        ...
        FROM vultrack.poam p
        LEFT JOIN vultrack.user u ON p.submitterId = u.userId
      `;
            let params = [];
    ...
    try {
        return await withConnection(async (connection) => {
            let sql = `
                ...
                FROM vultrack.poam T1
                LEFT JOIN vultrack.user T2 ON T1.submitterId = T2.userId
                WHERE poamId = ?;
                `;
...
exports.postPoam = async function postPoam(req) {
    ...
    let sql_query = `INSERT INTO vultrack.poam (collectionId, vulnerabilitySource, stigTitle, stigBenchmarkId, stigCheckData,
    ...
    try {
        return await withConnection(async (connection) => {
            if (!req.body.officeOrg) {
                let userSql = "SELECT officeOrg, fullName, email FROM vultrack.user WHERE userId = ?";
            ...
            let sql = "SELECT * FROM vultrack.poam WHERE poamId = LAST_INSERT_ID();";
            ...
            if (req.body.assignees) {
                ...
                for (let user of assignees) {
                    ...
                    let sql_query = `INSERT INTO vultrack.poamassignees (poamId, userId) values (?, ?)`;
            ...
            if (req.body.approvers) {
                ...
                for (let user of approvers) {
                    ...
                    let sql_query = `INSERT INTO vultrack.poamapprovers (poamId, userId) values (?, ?)`;
                    await connection.query(sql_query, [poam.poamId, user.userId]);
                }
            }
            ...
            if (req.body.assets) {
                ...
                for (let asset of assets) {
                    if (asset.assetId) {
                        let sql_query = `INSERT INTO vultrack.poamassets (poamId, assetId) VALUES (?, ?)`;
                        await connection.query(sql_query, [poam.poamId, asset.assetId]);
                    } else if (asset.assetName) {
                        let sql_query_check = `SELECT assetId FROM vultrack.asset WHERE assetName = ? AND collectionId = ?`;
                        ...
                        if (existingAsset.length > 0) {
                            ...
                            let sql_query2 = `INSERT INTO vultrack.poamassets (poamId, assetId) VALUES (?, ?)`;
                            await connection.query(sql_query2, [poam.poamId, assetId]);
                        } else {
                            let sql_query_insert = `INSERT INTO vultrack.asset (assetName, collectionId) VALUES (?, ?)`;
                            let [rowAsset] = await connection.query(sql_query_insert, [asset.assetName, poam.collectionId]);
                            let assetId = rowAsset.insertId;
                            let sql_query_insert_poamasset = `INSERT INTO vultrack.poamassets (poamId, assetId) VALUES (?, ?)`;
...
exports.putPoam = async function putPoam(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            const [existingPoamRow] = await connection.query("SELECT * FROM vultrack.poam WHERE poamId = ?", [req.body.poamId]);
            ...
            const sqlInsertPoam = `UPDATE vultrack.poam SET collectionId = ?, vulnerabilitySource = ?, stigTitle = ?, stigBenchmarkId = ?, stigCheckData = ?,
            ...
            const [updatedPoamRow] = await connection.query("SELECT * FROM vultrack.poam WHERE poamId = ?", [req.body.poamId]);
            ...
            if (req.body.status === 'Submitted') {
                let sql = `
    SELECT pa.userId
    FROM vultrack.poamapprovers pa
    JOIN vultrack.collectionpermissions cp ON pa.userId = cp.userId
  ...
  `;
                ...
                const notificationPromises = poamApprovers.map(async (approver) => {
                    ...
                    const notificationSql = `INSERT INTO vultrack.notification (userId, title, message) VALUES (?, ?, ?)`;
...
exports.updatePoamStatus = async function updatePoamStatus(req, res, next) {
    if (!req.params.poamId) {
        ...
        try {
            return await withConnection(async (connection) => {
                const [existingPoamRow] = await connection.query("SELECT * FROM vultrack.poam WHERE poamId = ?", [req.params.poamId]);
                ...
                const sqlUpdatePoam = `UPDATE vultrack.poam SET status = ? WHERE poamId = ?`;
                await connection.query(sqlUpdatePoam, [req.body.status, req.params.poamId]);

                const [updatedPoamRow] = await connection.query("SELECT * FROM vultrack.poam WHERE poamId = ?", [req.params.poamId]);
                ...
                if (req.body.status === 'Submitted') {
                    let sql = "SELECT * FROM vultrack.poamapprovers WHERE poamId = ?";
    ...
    exports.deletePoam = async function deletePoam(req, res, next) {
        ...
        try {
            await withConnection(async (connection) => {
                ...
                let sqlDeleteAssets = "DELETE FROM vultrack.poamassets WHERE poamId = ?;";
                await connection.query(sqlDeleteAssets, [req.params.poamId]);

                let sqlDeletePoam = "DELETE FROM vultrack.poam WHERE poamId = ?;";
