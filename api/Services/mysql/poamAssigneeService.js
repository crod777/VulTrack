...
exports.getPoamAssignees = async function getPoamAssignees() {
    return await withConnection(async (connection) => {
        let sql = `
            ...
            FROM vultrack.poamassignees t1
...
exports.postPoamAssignee = async function postPoamAssignee(req, res, next) {
    ...
    return await withConnection(async (connection) => {
        try {
            let fetchSql = "SELECT poamId, userId FROM vultrack.poamassignees WHERE userId = ? AND poamId = ?";
            ...
            let addSql = "INSERT INTO vultrack.poamassignees (poamId, userId) VALUES (?, ?)";
            ...
            let fetchNewSql = "SELECT poamId, userId FROM vultrack.poamassignees WHERE userId = ? AND poamId = ?";
...
exports.deletePoamAssignee = async function deletePoamAssignee(req, res, next) {
    ...
    await withConnection(async (connection) => {
        const userSql = "SELECT fullName FROM vultrack.user WHERE userId = ?";
        const [user] = await connection.query(userSql, [req.params.userId]);
        const fullName = user[0] ? user[0].fullName : "Unknown User";

        let sql = "DELETE FROM vultrack.poamassignees WHERE userId = ? AND poamId = ?";
