...
exports.getAssets = async function getAssets(req, res, next) {
    try {
        return await withConnection(async (connection) => {
            let sql = "SELECT * FROM  vultrack.asset ORDER BY assetName;"
...
exports.getAssetsByCollection = async function getAssetsByCollection(req, res, next) {
    try {
        ...
        return await withConnection(async (connection) => {
            const sql = "SELECT * FROM vultrack.asset WHERE collectionId = ? ORDER BY assetName;";
...
exports.getAsset = async function getAsset(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            const sql = "SELECT * FROM  vultrack.asset WHERE assetId = ?";
...
exports.getAssetByName = async function getAssetByName(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            const sql = "SELECT * FROM vultrack.asset WHERE assetName = ?";
...
exports.postAsset = async function postAsset(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql_query = `
                INSERT INTO vultrack.asset (assetName, fullyQualifiedDomainName,
            ...
            let sql = "SELECT * FROM vultrack.asset WHERE assetName = ?";
            let [rowAsset] = await connection.query(sql, [req.body.assetName]);
            if (req.body.labels) {
                let labels = req.body.labels;
                for (let label of labels) {
                    let sql_query = `
                        INSERT INTO vultrack.assetLabels (assetId, labelId) 
                        VALUES (?, ?)
                    `;
...
exports.putAsset = async function putAsset(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            ...
            let sql = "SELECT * FROM vultrack.asset WHERE assetId = ?";
...
exports.deleteAsset = async function deleteAsset(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql = "DELETE FROM vultrack.asset WHERE assetId = ?";
...
exports.deleteAssetsByPoamId = async function deleteAssetsByPoamId(req, res, next) {
    ...
    try {
        await withConnection(async (connection) => {
            let findAssetSql = "SELECT * FROM vultrack.poamassets WHERE poamId = ?";
            ...
            if (assetIds.length > 0) {
                let deleteSql = "DELETE FROM vultrack.asset WHERE assetId IN (?)";
