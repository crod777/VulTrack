...
exports.getLabels = async function getLabels(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql = "SELECT * FROM vultrack.label WHERE collectionId = ? ORDER BY labelName;";
...
exports.getLabel = async function getLabel(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql = "SELECT * FROM vultrack.label WHERE labelId = ? AND collectionId = ?";
...
exports.postLabel = async function postLabel(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql_query = `INSERT INTO vultrack.label (labelName, description, collectionId) VALUES (?, ?, ?)`;
            await connection.query(sql_query, [req.body.labelName, req.body.description, req.params.collectionId]);

            let sql = "SELECT * FROM vultrack.label WHERE labelName = ? AND collectionId = ?";
...
exports.putLabel = async function putLabel(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql_query = "UPDATE vultrack.label SET labelName = ?, description = ? WHERE labelId = ? AND collectionId = ?";
...
exports.deleteLabel = async function deleteLabel(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql = "DELETE FROM vultrack.label WHERE labelId = ? AND collectionId = ?";
