    ...
    try {
        return await withConnection(async (connection) => {
            let sql = `
                ...
                FROM vultrack.poamassets t1
                INNER JOIN vultrack.asset t2 ON t1.assetId = t2.assetId
            `;
...
exports.deletePoamAssetByPoamId = async function deletePoamAssetByPoamId(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql = "DELETE FROM vultrack.poamassets WHERE poamId = ?";
...
exports.postPoamAsset = async function postPoamAsset(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql_query = `INSERT INTO vultrack.poamassets (assetId, poamId) VALUES (?, ?)`;
            ...
            let assetNameQuery = `SELECT assetName FROM vultrack.asset WHERE assetId = ?`;
        ...
        if (error.code === 'ER_DUP_ENTRY') {
            return await withConnection(async (connection) => {
                let fetchSql = "SELECT * FROM vultrack.poamassets WHERE assetId = ? AND poamId = ?";
...
exports.deletePoamAsset = async function deletePoamAsset(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql = "DELETE FROM vultrack.poamassets WHERE assetId = ? AND poamId = ?";
