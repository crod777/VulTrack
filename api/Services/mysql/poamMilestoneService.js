...
exports.getPoamMilestones = async function getPoamMilestones(poamId) {
    try {
        ...
        return await withConnection(async (connection) => {
            let sql = "SELECT * FROM vultrack.poammilestones WHERE poamId = ?;";
...
exports.postPoamMilestone = async function postPoamMilestone(poamId, requestBody) {
    try {
        ...
        return await withConnection(async (connection) => {
            let sql_query = `INSERT INTO vultrack.poamMilestones (poamId, milestoneDate, milestoneComments, milestoneStatus) VALUES (?, ?, ?, ?)`;
...
exports.putPoamMilestone = async function putPoamMilestone(poamId, milestoneId, requestBody) {
    try {
        ...
        return await withConnection(async (connection) => {
            let getMilestoneSql = "SELECT * FROM vultrack.poammilestones WHERE poamId = ? AND milestoneId = ?";
            let [existingMilestone] = await connection.query(getMilestoneSql, [poamId, milestoneId]);

            let sql_query = `UPDATE vultrack.poammilestones SET milestoneDate = ?, milestoneComments = ?, milestoneStatus = ? WHERE poamId = ? AND milestoneId = ?`;
            await connection.query(sql_query, [
                requestBody.milestoneDate,
                requestBody.milestoneComments,
                requestBody.milestoneStatus,
            ...
            sql_query = "SELECT * FROM vultrack.poamMilestones WHERE poamId = ?;";
...
exports.deletePoamMilestone = async function deletePoamMilestone(poamId, milestoneId, requestBody) {
    try {
        ...
        return await withConnection(async (connection) => {
            let sql = "DELETE FROM vultrack.poammilestones WHERE poamId= ? AND milestoneId = ?";
