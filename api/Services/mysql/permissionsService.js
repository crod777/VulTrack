...
exports.getCollectionPermissions = async function getCollectionPermissions(req, res, next) {
    ...
    try {
        const permissions = await withConnection(async (connection) => {
            let sql = "SELECT T1.*, T2.firstName, T2.lastName, T2.fullName, T2.email FROM vultrack.collectionpermissions T1 " +
                "INNER JOIN vultrack.user T2 ON t1.userId = t2.userId WHERE collectionId = ?;"
...
exports.postPermission = async function postPermission(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql_query = "INSERT INTO vultrack.collectionpermissions (accessLevel, userId, collectionId) VALUES (?, ?, ?);";
        ...
        if (error.code === 'ER_DUP_ENTRY') {
            return await withConnection(async (connection) => {
                let fetchSql = "SELECT * FROM vultrack.collectionpermissions WHERE userId = ? AND collectionId = ?";
...
exports.putPermission = async function putPermission(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql_query = "UPDATE vultrack.collectionpermissions SET collectionId = ?, accessLevel = ? WHERE userId = ? AND collectionId = ?;";
...
exports.deletePermission = async function deletePermission(req, res, next) {
    ...
    try {
        return await withConnection(async (connection) => {
            let sql = "DELETE FROM  vultrack.collectionpermissions WHERE userId = ? AND collectionId = ?";
