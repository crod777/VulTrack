...
@Component({
  selector: 'vultrack-unauthorized',
...
export class UnauthorizedComponent implements OnInit {
  ...
  async ngOnInit() {
    const isAuthenticated = await this.authService.isAuthenticated('vultrack');
    ...
    if (isAuthenticated && isStigmanAuthenticated) {
      ...
      await this.authService.login('vultrack');
    } else if (isAuthenticated && !isStigmanAuthenticated) {
      await this.authService.login('stigman');
    } else {
      await this.authService.login('stigman');
      await this.authService.login('vultrack');
    }
