...
export class AuthService {
  ...
  async initializeAuthentication(): Promise<void> {    
    ...
    const isAuthenticatedCpat = authResult.find(auth => auth.configId === 'vultrack')?.isAuthenticated;

    if (!isAuthenticatedStigman) {
      await this.oidcSecurityService.authorize('stigman');
    } else if (!isAuthenticatedCpat) {
      await this.oidcSecurityService.authorize('vultrack');
    } else {
      (await this.usersService.loginState("logIn")).subscribe((result: any) => console.log("[VUL-TRACK] ", result.message));
  ...
  async logout() {
    await (await this.usersService.loginState("logOut")).subscribe((result: any) => console.log("[VUL-TRACK] ", result.message));
    await this.oidcSecurityService.logoff('stigman', undefined);
    await this.oidcSecurityService.logoff('vultrack', undefined).subscribe((result) => console.log(result));
  }
}
